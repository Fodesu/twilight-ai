...
// An OpenAI-compatible server may answer without id, model or created. Neither
...
func TestBareReplyCarriesNoResponseMetadata(t *testing.T) {
	...
	defer srv.Close()
	model := &sdk.Model{ID: "local", Provider: completions.New(completions.WithAPIKey("k"), completions.WithBaseURL(srv.URL))}
	req := sdk.Request{Model: "local", Messages: []sdk.Message{sdk.UserMessage("hi")}}
