...
func TestRegressionToolStepIDReproducible(t *testing.T) {
	...
	if schema.V1().Identity.DeriveToolStepID(opened.Source, opened.BindingSetDigest) != opened.StepID {
		t.Fatal("ToolStepOpened digest does not reproduce its StepID")
	}
	s = fold(t, s, facts)
	ts := s.Current.(run.ToolStep)
	if schema.V1().Identity.DeriveToolStepID(ts.Source, ts.RefValue.Digest) != ts.RefValue.ID {
...
func TestRegressionCancelReasonFixed(t *testing.T) {
	s := newRun(t)
	if _, err := schema.V1().Machine.Decide(s, run.CancelRun{Reason: run.RunReason("other")}); err == nil {
