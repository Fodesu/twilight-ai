...
func modelEffect(sch schema.Schema, runID run.RunID, step *run.ModelStep) effectRef {
	return effectRef{schema: sch, runID: runID, id: sch.Identity.DeriveEffectID(runID, step.RefValue.ID, "", step.Rejects)}
}

// toolEffect is the one tool effect of a call: a call starts at most once.
func toolEffect(sch schema.Schema, runID run.RunID, stepID run.StepID, callID run.CallID) effectRef {
	return effectRef{schema: sch, runID: runID, id: sch.Identity.DeriveEffectID(runID, stepID, callID, 0)}
}

func (e *effectRef) startID() run.CommandID { return e.schema.Identity.DeriveStartCommandID(e.id) }
func (e *effectRef) settlementID() run.CommandID {
	return e.schema.Identity.DeriveSettlementCommandID(e.id)
}
func (e *effectRef) recoveryID() run.CommandID {
	return e.schema.Identity.DeriveRecoveryCommandID(e.id)
}
...
func (l *Loop) settle(ctx context.Context, rt runtime.RunStore, events EventSink, e *effectRef, base run.RunPosition, cmd run.AgentCommand, sch schema.Schema) (*run.RunResult, error) {
	id := e.settlementID()
	if _, recovering := cmd.(run.RecoverModelExecution); recovering {
		id = e.recoveryID()
	}
	res, err := l.commit(context.WithoutCancel(ctx), rt, e.runID, id, base, cmd, sch)
	if err != nil {
		if retriable(err) {
			return nil, nil
		}
