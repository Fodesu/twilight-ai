...
// none of it. Next is the at-most-one pending effect of a Run (RUN-MCH-4),
// with the read-only queries WaitingCalls, ExecutingCalls and NeedsRecovery
// the application inspects when Next has no executable effect; the Loop
// re-derives the effect after every Load. RecoveryTargets, RecoveryCommand
