...
func TestCommandEnvelopeJSONRoundTripRestoresVariants(t *testing.T) {
	commands := []run.AgentCommand{
		...
		run.StartModelExecution{StepID: "s", Claim: "claim-s"},
		run.RecoverModelExecution{StepID: "s", Claim: "claim-s"},
		run.SubmitModelResult{StepID: "s", Result: model.ModelResult{Text: "ok"}},
		run.SubmitModelFailure{StepID: "s", Failure: run.StepFailure{Class: run.FailureProvider, Message: "down"}},
		run.RejectModelResult{StepID: "s", Usage: model.Usage{TotalTokens: 1}, Failure: run.StepFailure{Class: run.FailureMalformedModel}},
		run.StartToolCall{StepID: "ts", CallID: "c", Claim: "claim-c"},
		run.SubmitToolResult{StepID: "ts", CallID: "c", Result: run.ToolExecutionResult{Output: cj(`{"ok":true}`)}},
		run.SubmitToolFailure{StepID: "ts", CallID: "c", Failure: run.ToolFailure{Class: run.FailureExecution}, Outcome: run.ToolOutcomeKnown},
...
func TestFactCodecRoundTripRestoresVariants(t *testing.T) {
	facts := []run.Fact{
		run.RunCreated{RunID: "run-1", Owner: "turn-1", Attempt: 1, CausationID: "cause"},
		...
		run.ModelStepStarted{StepID: "s", Claim: "claim-m"},
		...
		run.ToolCallStarted{StepID: "ts", CallID: "c", Claim: "claim-t"},
