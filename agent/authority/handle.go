...
// turn.Commands, chatlog.Commands, driver.Drive -- takes its Writer, so all
// authoritative writes of the Session land on one Writer, one epoch and one
// projection view, and the Writer fences a stale owner. Reads take a
// SessionID and need no Handle.
type Handle struct {
...
func (a *Authority) Open(ctx context.Context, sid session.SessionID) (*Handle, error) {
	...
	return &Handle{Recovered: n, w: w, a: a}, nil
}

// ID is the Session this Handle owns.
func (h *Handle) ID() session.SessionID { return h.w.SessionID() }
...
// than a name.
func (h *Handle) Writer() writer.Writer { return h.w }
