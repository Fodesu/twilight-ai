...
// Advance is SES-ADV-1/2: it seals a child segment of the tip at the current
// head together with its bootstrap commits, publishes it as the root's tip
// through one Backend step, and moves the handle onto it. It takes the
// Ledger's graph lock like Create: the new node must not be seen by a
// Collect before the root names it.
func (w *ledgerHandle) Advance(ctx context.Context, req AdvanceRequest) (SegmentHeader, []Commit, error) {
	if err := ctx.Err(); err != nil {
	...
	defer w.mu.Unlock()
	if w.failed != nil {
		return SegmentHeader{}, nil, w.failed
	}
	if w.head.Next == 0 {
		return SegmentHeader{}, nil, newError(ErrInvalid, "advance", sid, "the tip holds no commit to anchor the new segment to")
	}
	// The edge names the segment that contributes the head commit
	// (SES-FRK-1): the tip when it has own commits, an ancestor otherwise.
	anchor := w.head.Next - 1
	owner, ok := w.ancestry.Owner(anchor)
	if !ok {
		return SegmentHeader{}, nil, newError(ErrCorrupt, "advance", sid, fmt.Sprintf("no segment contributes commit %d", anchor))
	}
	header := SegmentHeader{ProtocolVersion: w.profile.Version(), CausationID: req.CausationID, Metadata: req.Metadata,
		Parent: &LedgerRef{Segment: owner.Segment.ID, Seq: anchor, Digest: w.head.Digest}}
	nonce, err := w.l.nonce()
	if err != nil {
		return SegmentHeader{}, nil, err
	}
	header.Nonce = nonce
	digest, err := w.profile.HeaderDigest(header)
	if err != nil {
		return SegmentHeader{}, nil, err
	}
	header.HeaderDigest = digest
	if err := w.profile.ValidateHeader(header); err != nil {
		return SegmentHeader{}, nil, err
	}
	segment := Segment{ID: SegmentIDOf(header), Header: header}
	// Bootstrap commits are sealed from the seed exactly as Append would
	// seal them one by one (SES-APP-1/3); their CommitIDs must be new to the
	// whole history the new tip inherits (SES-FRK-3).
	head := segment.Seed()
	commits := make([]Commit, 0, len(req.Bootstrap))
	seen := make(map[CommitID]struct{}, len(req.Bootstrap))
	for i := range req.Bootstrap {
		p := &req.Bootstrap[i]
		if err := validIdentity("CommitID", string(p.CommitID)); err != nil {
			return SegmentHeader{}, nil, newError(ErrInvalid, "advance", sid, err.Error())
		}
		if err := ValidateBatches(p.Batches); err != nil {
			return SegmentHeader{}, nil, newError(ErrInvalid, "advance", sid, err.Error())
		}
		if _, dup := seen[p.CommitID]; dup || w.committedLocked(p.CommitID) {
			return SegmentHeader{}, nil, &Error{Code: ErrConflict, Operation: "advance", SessionID: sid, CommitID: p.CommitID, Detail: "CommitID already in the ledger"}
		}
		seen[p.CommitID] = struct{}{}
		c := Commit{Seq: head.Next, CommitID: p.CommitID, Epoch: w.lease.Epoch, Intent: p.Intent, Batches: cloneBatches(p.Batches)}
		if err := SealCommit(w.profile, head.Digest, segment.ID, &c); err != nil {
			return SegmentHeader{}, nil, err
		}
		commits = append(commits, c)
		head = Head{Next: c.Seq + 1, Digest: c.Digest}
	}
	if err := w.l.be.AdvanceTip(ctx, w.lease, segment, commits, w.root.Tip); err != nil {
		return SegmentHeader{}, nil, err
	}
	// Published: the handle now owns the new tip. Its own index is the
	// bootstrap; the previous tip's own commits are inherited prefix.
	w.root.Tip = segment.ID
	a, err := LoadAncestry(ctx, w.l.be, segment.ID)
	if err != nil {
		// The publication succeeded; a failure to reload the path leaves the
		// handle unable to answer for the new tip, so it fails closed.
		w.failed = &Error{Code: ErrHandleFailed, Operation: "advance", SessionID: sid, Detail: err.Error()}
		return header, commits, w.failed
	}
	w.ancestry = a
	w.head = head
	w.own = make(map[CommitID]struct{}, len(commits))
	w.streams = make(map[StreamRef]StreamSeq)
	for i := range commits {
		w.own[commits[i].CommitID] = struct{}{}
		w.countStreams(&commits[i])
	}
	return header, commits, nil
}
