...
// LocalExecutor behind a durable Worker with an HTTP data plane) -- over one
// shared file store, and assert the protocols the specs make across real
// process boundaries: an executor crash settles Unknown without re-running
// the effect, a takeover reattaches to a still-running effect, a fenced
