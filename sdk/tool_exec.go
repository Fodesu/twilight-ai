	...
	for i, tc := range calls {
		if !tc.Input.Valid() {
			...
			results[i] = ToolResultPart{
				...
				Result:     TextOutput(fmt.Sprintf("invalid tool arguments for %q: not a JSON document", tc.ToolName)),
				IsError:    true,
			}
			continue
		}
		...
		if !ok || tool.Execute == nil {
			results[i] = ToolResultPart{
				ToolCallID: tc.ToolCallID,
				ToolName:   tc.ToolName,
				Result:     TextOutput(fmt.Sprintf("tool %q not found or has no execute handler", tc.ToolName)),
				IsError:    true,
			}
			continue
		}
		...
		if tool.RequireApproval {
			if opts.Approve == nil {
				...
				results[i] = ToolResultPart{
					ToolCallID: tc.ToolCallID,
					ToolName:   tc.ToolName,
					Result:     TextOutput("tool execution denied: no approval handler"),
					IsError:    true,
				}
				continue
			}
			...
			case ToolApprovalDecisionRejected:
				...
				results[i] = ToolResultPart{
					ToolCallID: tc.ToolCallID,
					ToolName:   tc.ToolName,
					Result:     TextOutput(rejectedToolResultText(approval)),
					IsError:    true,
				}
				continue
...
func ToolCallResults(calls []ToolCall, parts []ToolResultPart) []ToolResult {
	inputs := make(map[string]ToolArguments, len(calls))
	for _, c := range calls {
...
func runTool(ctx context.Context, tc *ToolCall, tool *Tool, sendProgress func(StreamPart)) ToolResultPart {
	var progressFn func(content ToolOutput)
	if sendProgress != nil {
		progressFn = func(content ToolOutput) {
