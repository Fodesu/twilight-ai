...
**AUTH-FRK-3** `DeleteSession` 先停止该 Session 的恢复监听并关闭其 Writer，再以 `writer.Delete` 撤根（EXT-WRT-9）；被另一进程持有的 Session 为 `ErrOwned`。以它为前缀的 fork 不受影响，前缀的 claim 随段保留。`Collect` 调 `writer.Collect` 回收无根可达的段并释放被回收 commit 的 claim（SES-GC-2/3）。
