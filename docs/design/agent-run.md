...
type Watcher struct { Port ExecutionPort; Poll, Reconnect time.Duration } // Owner 侧：一条 Settlements 订阅 + 周期性读取，服务多个 key 的等待；Watch(ctx, key, deliver, fail) 登记一个 key
func AwaitOutcome(ctx context.Context, port ExecutionPort, key AssignmentKey, poll time.Duration) (Outcome, error) // 同步形态：先订阅再读，供单个 effect 的调用方
