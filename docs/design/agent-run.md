...
**RUN-EXE-15（重派与 dispatch ledger）** Session 与 Executor 之间没有共享事务：Loop 提交 start 事实后向 Executor `Dispatch`，两步之间的崩溃留下一个 Session 已记录为 Executing、Executor 从未收到的 effect。这个空窗由接管处置处理（RUN-CMT-7）：Reconciler 就是两个 authority 之间的 process manager，它读三处状态——Run 机器给出 Executing 的 effect（`plan.RecoveryTargets`，带 EffectID），Executor 的 `Attach` 给出 attempt 是否存在，dispatch ledger（`agentcore/process`）给出该 effect 已被重派几次、是否放弃。三者各是自己事实的唯一来源，process 不转录另外两方的观察。dispatch ledger 每个 effect 一条，事实只有两种：`dispatch_attempted{attempt}`（attempt 从 1 连续编号）与 `given_up{reason}`；折叠为 `{Attempts, GivenUp, Reason}`；ledger 规则与 RUN-EXE-14 相同（Seq、CommitID 重放、写前折叠），fence 为 Session owner 的 Epoch（`Append(epoch, key, commit)`，更小的 epoch 为 `ErrFenced`）。对 `missing` 的判定：已放弃或 `Attempts` 达到预算（`MaxRedispatches`，默认 3）则记 `given_up` 并处置；否则先记 `dispatch_attempted`，再经 `loop.Redispatch` 从 Run 状态重建 Assignment 交给 Executor（Executor 按 key 识别重放，RUN-EXE-3）。先记后派，两步之间的崩溃只消耗一次预算，不会有 ledger 不知道的 Dispatch。`Redispatch` 成功为 verdict `redispatch`，目标保持 Executing 并像 `keep` 一样等待 Outcome；`ErrDispatchRetryable` 或 `ErrDispatchUnknown` 为 `defer`，下一次接管处置再问；其他错误（含 `ErrEffectNotExecuting`）记 `given_up` 并处置。没有装配 dispatch ledger 时 `missing` 保持只处置。local agent 与 cloud agent 运行同一个 Reconciler，差别只在 port：dispatch ledger 在本地 SQLite 文件或共享数据库，Executor 是进程内 Worker 或 HTTP 远端。
